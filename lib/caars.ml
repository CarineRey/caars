...
let seq_dispatcher
  ...
  workflow ~np:threads ~version:11 ~descr:("SeqDispatcher.py:" ^ query_id ^ "_" ^ query_species) [
    mkdir_p tmp;
...
let apytram_checked_families_of_orfs_ref_fams apytram_orfs_ref_fams configuration_dir ref_blast_dbs =
 List.map apytram_orfs_ref_fams ~f:(fun (fam, fws) ->
  let checked_fws = List.map fws ~f:(fun (s, f, apytram_orfs_fasta) ->
    ...
    let seq2fam = concat ~descr:(descr_ref ^ ".seq2fam") (List.map s.ref_species ~f:(fun r -> (configuration_dir / ref_seq_fam_links r))) in
    let ref_db = List.map s.ref_species ~f:(fun r -> List.Assoc.find_exn ~equal:( = ) ref_blast_dbs r) in
    let checked_families_fasta = checkfamily ~descr:(":"^s.id^"."^f.name) ~input ~family:f.name ~ref_transcriptome ~seq2fam ~ref_db ~evalue:1e-10 in
          ...
          let fws = List.concat (
                    ...
                    List.concat (
                      List.map pairs ~f:(fun (ref_species, fam) ->
                        ...
                        let compressed_reads_dbs = List.filter_map reads_blast_dbs ~f:(fun (s, db) -> if (s.ref_species = ref_species && s.apytram_group = apytram_group)then Some db else None) in
                        let time_max = 18000 * List.length compressed_reads_dbs in
                        let w = Apytram.apytram_multi_species ~descr ~time_max ~no_best_file:true ~write_even_empty:true ~mal:66 ~plot:false ~i:5 ~evalue:1e-10 ~out_by_species:true ~memory:divided_thread_memory ~fam:fam.name ~query compressed_reads_dbs in
                        List.filter_map configuration.apytram_samples ~f:(fun s ->
