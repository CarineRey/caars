...
let gene_tree fam o =
  Workflow.select o [ "Gene_trees" ; fam ^ ".tree" ]
...
let parse_input ~sample_sheet ~species_tree_file ~alignments_dir ~seq2sp_dir ~(all_families: family list) ~memory ~configuration : configuration_dir directory =
  ...
  let ali = Workflow.input (configuration.alignments_dir ^ "/" ^ fam.name ^ ".fa") in
  cmd "echo" [dep ali]
  )
  in
  ...
  let sp2seq = Workflow.input (configuration.seq2sp_dir ^ "/" ^ sp2seq_file) in
  cmd "echo" [dep sp2seq]
  )
  in
...
let ref_transcriptomes species o =
  Workflow.select o ["R_Sp_transcriptomes" ;  species ^ "_transcriptome.fa" ]

let ref_seq_fam_links species o =
  Workflow.select o ["R_Sp_Seq_Fam_links";  species ^ "_Fam_Seq.tsv"  ]

let ref_fams species family o =
  Workflow.select o ["R_Sp_Gene_Families"; species ^ "." ^ family ^ ".fa"]

let ali_species2seq_links family o =
  Workflow.select o ["Alignments_Species2Sequences" ; "alignments." ^  family ^ ".sp2seq.txt" ]

let ref_blast_dbs_of_configuration_dir {all_ref_species;_} configuration_dir =
  List.map all_ref_species ~f:(fun ref_species ->
    let fasta = ref_transcriptomes ref_species configuration_dir in
...
let fastq_to_fasta_conversion {all_ref_samples;_} =
  ...
  List.filter_map all_ref_samples ~f:(fun s ->
      ...
      in
      if need_rna then
        let sample_fasta_input = function
          | Fasta_Single_end (x, o) -> Fasta_Single_end (Workflow.input x, o)
          | Fasta_Paired_end (lw, rw, o) -> Fasta_Paired_end (Workflow.input lw, Workflow.input rw, o)
        in
        let sample_fastq_to_sample_fasta = function
          | Fastq_Single_end (w, o ) -> Fasta_Single_end ( Trinity.fastq2fasta ~descr:(s.id ^ "_" ^ s.species) ~dep_input (Workflow.input w) , o )
          | Fastq_Paired_end (lw, rw , o) -> Fasta_Paired_end ( Trinity.fastq2fasta ~descr:(s.id ^ "_" ^ s.species ^ "_left") ~dep_input (Workflow.input lw) , Trinity.fastq2fasta ~descr:(s.id ^ "_" ^ s.species ^ "_right") ~dep_input (Workflow.input rw) , o)
        in
        let sample_fasta = match s.sample_file with
            | Sample_fasta x -> sample_fasta_input x
  ...
  List.map fasta_reads ~f:(fun (s, fasta_sample) ->
      ...
      let norm_fasta_sample_to_normalization_dir normalization_dir = function
        ...
        | Fasta_Paired_end (_, _ , o) -> Fasta_Paired_end (Workflow.select normalization_dir ["left.norm.fa"],
                                                           Workflow.select normalization_dir ["right.norm.fa"], o )
      in
      (s, norm_fasta_sample_to_normalization_dir normalization_dir fasta_sample )
    )


let trinity_assemblies_of_norm_fasta norm_fasta {trinity_samples ; _} memory threads =
  List.concat [
    ...
    List.filter_map trinity_samples ~f:(fun s ->
        if s.given_assembly then
          Some (s, Workflow.input s.path_assembly)
        else
          None
      )
  ]

let transdecoder_orfs_of_trinity_assemblies trinity_assemblies { memory ; threads ; _ } =
...
let concat ?(descr="") = function
  ...
  | x :: [] -> x
  | fXs ->
    Workflow.shell ~descr:("concat" ^ descr) [
      cmd "cat" ~stdout:dest [ list dep ~sep:" " fXs ]
    ]


(* let is_in ?(descr="") ~string_to_test ~file =
 *     Workflow.shell ~descr:(string_to_test ^".is_in" ^ descr) [
...
let blast_dbs_of_norm_fasta norm_fasta =
  List.filter_map norm_fasta ~f:(fun (s, norm_fasta) ->
      if s.run_apytram then
        ...
        let rep_cluster_fasta = Workflow.select cluster_repo ["cluster_rep.fa"] in
        let cluster = Workflow.select cluster_repo ["cluster_rep.fa.clstr"] in
        (*reformat cluster*)
...
let trinity_annotated_fams_of_trinity_assemblies configuration_dir ref_blast_dbs threads=
  List.map ~f:(fun (s,trinity_assembly) ->
      let ref_db = List.map s.ref_species ~f:(fun r -> List.Assoc.find_exn ~equal:String.( = ) ref_blast_dbs r) in
...
let build_target_query ref_species family configuration trinity_annotated_fams apytram_group =
    let seq_dispatcher_results_dirs =
        List.filter_map configuration.apytram_samples ~f:(fun s ->
            if String.(s.apytram_group = apytram_group) && Poly.(s.ref_species = ref_species) && (s.run_trinity) then
                Some (s, List.Assoc.find_exn ~equal:Poly.( = ) trinity_annotated_fams s)
            else
                None
            )
    in
...
let merged_families_of_families configuration configuration_dir trinity_annotated_fams apytram_annotated_fams =
  List.map configuration.used_families ~f:(fun family ->
      ...
      let alignment = Workflow.input (configuration.alignments_dir ^ "/" ^ family.name ^ ".fa")  in
      let alignment_sp2seq = ali_species2seq_links family.name configuration_dir in
...
let phyldog_by_fam_of_merged_families merged_families configuration =
  List.map  merged_families ~f:(fun (fam, merged_without_filter_family, merged_and_filtered_family) ->
    ...
    let ali = Workflow.select merged_family [ fam.name ^ ".fa" ] in
...
let realign_merged_families merged_and_reconciled_families =
  List.map  merged_and_reconciled_families ~f:(fun (fam, reconciled_w, merged_w) ->
    (*let ali = merged_w / Workflow.select [ fam ^ ".fa" ] in
    let treein = reconciled_w / Workflow.select [ "Gene_trees/" ^ fam ^ ".ReconciledTree" ] in
    let threads = 1 in*)
...
let write_orthologs_relationships (merged_and_reconciled_families_dirs:'a workflow) configuration =
    let (ortho_dir,species_to_refine_list) = match configuration.run_reconciliation with
        | true -> (Some(Workflow.select merged_and_reconciled_families_dirs ["out/Orthologs_out"]),
...
let build_term configuration =
  ...
  let (apytram_memory, trinity_memory, trinity_threads) =
    if (List.length configuration.apytram_samples > 0) && (List.length configuration.trinity_samples > 0) then
      (Stdlib.( max 1 (configuration.memory * allocation_apytram / 100) ), Stdlib.(max 1 (configuration.memory * allocation_trinity / 100) ), Stdlib.( max 1 (configuration.threads * allocation_trinity / 100 )))
    else
  ...
  *)
  ...
  let (divided_sample_memory, divided_sample_threads) =
     ...
     (Stdlib.( max 1 (configuration.memory / (max 1 nb_samples)) ), Stdlib.(max 1 (configuration.threads / (max 1 nb_samples)) ))
    in
 ...
 *)
  ...
  let divided_thread_memory = Stdlib.(max 1 (configuration.memory / configuration.threads)) in

  let configuration_dir = parse_input ~sample_sheet:(Workflow.input configuration.sample_sheet)
                                                ~species_tree_file:(Workflow.input configuration.species_tree_file)
                                                ~alignments_dir:(Workflow.input configuration.alignments_dir)
                                                ~seq2sp_dir:(Workflow.input configuration.seq2sp_dir)
                                                ~all_families:configuration.all_families
                                                ~memory:divided_sample_memory
                                                ~configuration in

  let checked_used_families_all_together = check_used_families ~used_fam_list:configuration.used_families  ~usable_fam_file:(Workflow.select configuration_dir ["UsableFamilies.txt"]) in

  let ref_blast_dbs = ref_blast_dbs_of_configuration_dir configuration configuration_dir in

  let fasta_reads = fastq_to_fasta_conversion configuration in
 ...
 (* let apytram_annotated_ref_fams =
    ...
    List.map pairs ~f:(fun (s, fam) ->
        ...
        (s, fam, w / Workflow.select [ apytram_filename ] )
      )
  in
...
*)
                      ...
                      List.map pairs ~f:(fun (ref_species, fam) ->
                        ...
                        List.filter_map configuration.apytram_samples ~f:(fun s ->
                          if Poly.(s.ref_species = ref_species) && Poly.(s.apytram_group = apytram_group) then
                              ...
                              Some (s, fam, Workflow.select w [ apytram_filename ] )
                          else
                              None
                           )
                        )
  ...
  let repo = if configuration.just_parse_input || (List.length configuration.used_families) = 0 then
    List.concat [
      ...
      [ f ] %>  (Workflow.select configuration_dir [ f ]));
    ...
    List.concat [
      ...
      ;
       [["assembly_results_by_fam" ] %> (Workflow.select merged_reconciled_and_realigned_families_dirs ["out/"])]
      ;
      [["all_fam.seq2sp.tsv"] %> (Workflow.select orthologs_per_seq ["all_fam.seq2sp.tsv"])]
      ;
      [["plots"] %> final_plots];

      List.concat [
      match reconstructed_sequences with
        (*| Some w -> [["assembly_results_only_seq"] %> (w / Workflow.select ["assemblies/"]); ["assembly_results_by_fam";"Sp2Seq_out"; "all_fam.seq2sp.tsv"] %> (w / Workflow.select ["all_fam.seq2sp.tsv"])]*)
        | Some w -> [["assembly_results_only_seq"] %> (Workflow.select w ["assemblies/"])]
        | None -> []
      ]
      ;

      if configuration.run_reconciliation then
        [["all_fam.orthologs.tsv"] %> (Workflow.select orthologs_per_seq ["all_fam.orthologs.tsv"])]
      else
        []
      ;
      ...
      List.concat [
        ...
        ;
        List.map apytram_annotated_ref_fams_by_fam_by_groups ~f:(fun (_, fws) ->
        ...
        ;
        List.map apytram_checked_families ~f:(fun (_, fws) ->
            List.map fws ~f:(fun (s, fam, apytram_result) ->
  ...
  if configuration.just_parse_input then
    let repo = Repo.precious_item configuration_dir :: repo in
    Repo.to_workflow repo ~outdir:configuration.outdir
  else
