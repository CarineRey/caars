...
for line in f:
	...
	if len(line_list) == 7:
		rna_id,	sp,	ref_species, path_fastq, run_trinity, path_assembly, run_apytram = line_list
		...
		logger.error("Config file has not 7 elements in line:\n%s" %line)
		sys.exit(1)
...
def write_seq_ref_apytram(Ref_dic_trinity,AliDict_i,Family):
	for sp in Ref_dic_apytram.keys():
		#gene family:
		GeneFamily_File = "%s/%s_%s.fa" %(ApytramGeneFamDirPath,sp,Family)
		string = []
		for name in Ref_dic_trinity[sp]:
			seq = ''.join(AliDict_i[name]).replace("-","")
			string.extend([">",name,"\n",
