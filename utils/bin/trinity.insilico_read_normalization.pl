...
#  --trinity_dir <string>          :trinity directory
#
###############################################################################
...
main: {
    ...
    if ( (@left_files && @right_files) ||
        ...
        @checkpoints = ();
        &process_cmd("mkfifo $trinity_target_fa && cat left.fa right.fa > $trinity_target_fa &") unless (-s $trinity_target_fa && -e "$trinity_target_fa.ok");
        unless (not -p $trinity_target_fa){
            die "$trinity_target_fa is not a named pipe (FIFO)\n";
        }
...
sub make_normalized_reads_file {
    ...
    if ( $not_found_count ) {
        ...
        print STDERR "Warning , not all specified records have been retrieved (missing $not_found_count) from @source_files, see file: $outfile.missing_accs for list of missing entries";
...
sub run_nkbc_pairs_together {
    ...
    my $cmd = "trinity.nbkc_merge_left_right_stats.pl --left $left_stats_file --right $right_stats_file --sorted";
